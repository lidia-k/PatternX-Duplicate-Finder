#sn added pram "clipdet" to replace poorly named "size"
import torch
...
#a prevent error on next line (s1,s2,lable=line.strip().split('\t'):  
#    ValueError: not enough values to unpack (expected 3, got 1)
class DittoDataset(data.Dataset):
    """EM dataset"""

    def __init__(self, path,    max_len=256, size=None, 
                 clipdet=None, #sn new
                 lm='roberta',  da=None):
        self.tokenizer   = get_tokenizer(lm)
        self.pairs = []; self.labels = [];  self.max_len = max_len;  self.size = size
        self.clipdet = clipdet  #sn new.

        if isinstance(path, list): lines = path
        else:                      lines = open(path)
        for line in lines:
            if line.strip() == "":   continue  #sn new.  #a
            s1, s2, label = line.strip().split('\t')
            self.pairs.append((s1, s2))
            self.labels.append(int(label))

        self.pairs  = self.pairs[ :clipdet]  if clipdet > 0 else self.pairs  #sn was: self.pairs  = self.pairs[  :size]
        self.labels = self.labels[:clipdet]  if clipdet > 0 else self.labels #sn was: self.labels = self.labels[ :size]
        self.da = da
