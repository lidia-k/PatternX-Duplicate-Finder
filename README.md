...
- Usage: `python3 run.py --project penumbra --task m_training --npi`. 
    1) You can choose to use either the best model or a specific model. To specify which model to use, look at the line 92 of `src/DF_penumbra/training_magellan.py`. (TODO: Parametrize a model.)
    2) If you want to train a model with the labeled data excluding NPIs, remove `--npi` from the command: `python3 run.py --project penumbra --task m_training`
- Method: 
